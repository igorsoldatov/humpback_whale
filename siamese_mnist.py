...
class BalancedMNISTPair(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(imgs)):
            img = Image.fromarray(imgs[i].numpy().reshape((28, 28)), mode='L')
    ...
    def download(self):
        ...
        training_set = loadlocal_mnist(
            images_path=os.path.join(self.root, self.raw_folder, 'train-images-idx3-ubyte'),
            labels_path=os.path.join(self.root, self.raw_folder, 'train-labels-idx1-ubyte'))
        test_set = loadlocal_mnist(
            images_path=os.path.join(self.root, self.raw_folder, 't10k-images-idx3-ubyte'),
            labels_path=os.path.join(self.root, self.raw_folder, 't10k-labels-idx1-ubyte'))
        # training_set = (
