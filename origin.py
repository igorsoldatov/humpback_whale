...
TRAIN = '../DATASET/humpback_whale/all/train/'
TEST = '../DATASET/humpback_whale/all/test/'
...
def get_hash_to_images(p2h_):
    h2ps = {}
    for image_name, h in p2h_.items():
        if h not in h2ps:
            h2ps[h] = []
        if image_name not in h2ps[h]:
            h2ps[h].append(image_name)
    return h2ps


def get_prefer_image_for_hash(h2ps_):
    h2p = {}
    for h, ps in h2ps_.items():
        h2p[h] = prefer(ps)
    return h2p
...
p2h = get_image_hashs()
h2ps = get_hash_to_images(p2h)
h2p = get_prefer_image_for_hash(h2ps)

for k in h2ps:
    if len(h2ps[k]) > 1:
        print(f'{k}: {h2ps[k]}')

print(f'{len(h2p)}, {list(h2p.items())[:5]}')
...
def build_model(lr, l2, activation='sigmoid'):
    ...
    x = MaxPooling2D((2, 2), strides=(2, 2), name='maxpool1')(x)  # 96x96x64
    for i in range(2):
        x = BatchNormalization(name=f'bn{i+1}')(x)
        x = Conv2D(64, (3, 3), activation='relu', **kwargs)(x)

    x = MaxPooling2D((2, 2), strides=(2, 2), name='layer1.maxpool')(x)  # 48x48x64
    x = BatchNormalization()(x)
    x = Conv2D(128, (1, 1), activation='relu', **kwargs)(x)  # 48x48x128
    for _ in range(4):
        x = subblock(x, 64, **kwargs)

    x = MaxPooling2D((2, 2), strides=(2, 2), name='layer2.maxpool')(x)  # 24x24x128
    x = BatchNormalization()(x)
    x = Conv2D(256, (1, 1), activation='relu', **kwargs)(x)  # 24x24x256
    for _ in range(4):
        x = subblock(x, 64, **kwargs)

    x = MaxPooling2D((2, 2), strides=(2, 2), name='layer3.maxpool')(x)  # 12x12x256
    x = BatchNormalization()(x)
    x = Conv2D(384, (1, 1), activation='relu', **kwargs)(x)  # 12x12x384
    for _ in range(4):
        x = subblock(x, 96, **kwargs)

    x = MaxPooling2D((2, 2), strides=(2, 2), name='layer4.maxpool')(x)  # 6x6x384
    x = BatchNormalization()(x)
...
if 0:
    ...
    for l in model.layers[2].layers:
        ...
        for w in l.weights:
            nn = 1
            for s in list(w.shape):
                nn = nn * s.value
            nn_total += nn
        print(f'{l.name} : {l.input_shape[len(l.input_shape)-1]} : {l.output_shape[len(l.output_shape)-1]} : {nn_total}')


h2ws = {}
new_whale = 'new_whale'
for p, w in train_dict.items():
...
data = TrainingData(score, batch_size=64)
(a, b), c = data[0]
...
if False and isfile('./piotte/mpiotte-standard.model'):
    ...
    make_steps(10, 1000)
    ampl = 100.0
    model.save(f'./submissions/baseline-ep{steps}.model')
    for _ in range(2):
